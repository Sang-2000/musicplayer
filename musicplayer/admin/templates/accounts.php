...
if ($user)
{
    ...
    else if ($data_user['position'] == 1) 
    {
                    ...
                    <p class="form-add-acc">
                        <form method="POST" id="formAddAcc" onsubmit="return false;">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control title" id="username_add_acc">
                            </div>
                            <div class="form-group">
                                <label>Mật khẩu</label>
                                <input type="password" class="form-control title" id="password_add_acc">
                            </div>
                            <div class="form-group">
                                <label>Nhập lại mật khẩu</label>
                                <input type="password" class="form-control title" id="repasswprd_add_acc">
                            </div>
                            <div class="form-group">
        ...
        else
        {
            ...
            $sql_get_list_acc = "SELECT * FROM accounts";
            // Nếu có tài khoản
            if ($db->db_num_rows($sql_get_list_acc))
            {
                echo
                '
                    <br><br>
                ...
                foreach ($db->db_fetch_assoc($sql_get_list_acc, 0) as $key => $data_acc)
                {
                    // Trạng thái tài khoản
                    if ($data_acc['status'] == 0) {
                        $stt_acc = '<label class="label label-success">Hoạt động</label>';
                    } else if ($data_acc['status'] == 1) {
                        $stt_acc = '<label class="label label-danger">Khoá</label>';
                    }
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a data-id="' . $data_acc['id_acc'] . '" class="btn btn-sm btn-success unlock-acc-list">
                                    <span class="glyphicon glyphicon-lock"></span> Mở
                                </a>
